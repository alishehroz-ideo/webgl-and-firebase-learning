...
- **Firebase console** (screen-record this): the Realtime Database tree, expanding `assetCatalog` and `books` — proves the backend is real.
...
Framing: this is a **task submission to the reviewers**, not a product pitch — lead with what was built and prove it's real. Covers all six required points: ① overview · ② architecture · ③ asset loading & caching · ④ data saved/loaded · ⑤ key decisions & challenges · ⑥ AI tools.

---

### 1 · Title card  (~2s)
🎬 **SHOW:** A plain title slide — **"BookLab — Task 1: Interactive Content Creation"**, your name, and **"Adeeb · Phase 1 Technical Assessment."**
🎙 **SAY:**
> "This is my submission for BookLab — the Interactive Content Creation task from your Phase 1 assessment. I'll walk you through what I built, starting with the live backend on Firebase."

### 2 · Firebase backend — the live data  (~1:10)
🎬 **SHOW:** Screen-record the **Firebase console → Realtime Database**. Expand the tree as you talk: the two top branches → `assetCatalog` → `backgrounds` (open one item to show `name` + `url`) → `objects` (open one) → then `books` → `demo` → a saved book → `pages` → a page → `objects` → one placed object (show `x`, `y`, `scale`, `z`).
> ⚠️ Create a book or two in the app first, so the `books` node isn't empty when you expand it.
🎙 **SAY:**
> "This is the live database behind the app — Firebase Realtime Database. Everything the app uses is real and lives here; nothing is faked or baked into the build. There are two branches. The first is the asset catalog — the content library. Under 'backgrounds' is every scene a child can choose — each one just an id, a name, and a URL to the image — and under 'objects' are all the stickers, the same way. The app stores only these links, never the pictures themselves, so I can add or swap art just by editing this catalog, with no rebuild. The second branch is 'books' — the saved storybooks, organised per child. Under the demo child here is a saved book: a title, timestamps, and a list of pages. Open a page and it's simply a background id plus the objects placed on it. And each placed object is just a reference and a few numbers — which sticker, its position as normalized zero-to-one coordinates, its scale, and its draw order. So a whole book is a tiny recipe of references and numbers — never image data — and that's exactly what makes it quick to save, load, and replay on any screen size."

### 3 · Overview + live demo  (~1:30)   *(already recorded — keep as-is)*
...
### 5 · How data is saved & loaded  (~1:00)
...
### 7 · Key decisions & challenges  (~1:00)
